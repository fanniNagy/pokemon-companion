...
class MyFavouriteRepositoryTest {
    ...
    @Test
    public void myFavouriteSaveTest() {
        User user = getUser();
        ...
        List<PokemonEntity> testResult = myFavouriteRepository.findAll();
        assertNotNull(testPokemonEntity);
        assertTrue(testResult.contains(testPokemonEntity));
        assertEquals(testResult.get(0).getUserFavPokemons(), testPokemonEntity.getUserFavPokemons());
    }

    private PokemonEntity getPokemonEntity(User user) {
        Set<User> favoritePokemonUser = new HashSet<>();
        favoritePokemonUser.add(user);
        return PokemonEntity.builder()
                .name("test")
                .id(1l)
                .userFavPokemons(favoritePokemonUser)
                .build();
    }
